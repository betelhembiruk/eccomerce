      ...
      <div className='flex-1'>
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="border p-3 rounded-lg animate-pulse h-56 bg-gray-200"></div>
            ))}
          </div>
        ) : (
          <>
            ...
            <div className="flex justify-center mt-6 gap-2">
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`px-3 py-1 border ${currentPage === i + 1 ? 'bg-gray-700 text-white' : ''}`}
                >
                  {i + 1}
                </button>
              ))}
